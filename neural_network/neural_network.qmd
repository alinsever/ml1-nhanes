...
The model error rate increase as the number of daily drinks increases, which normal as those observation are underrepresentated in our dataset.
However, the mean error rate is around 2,14 drinks, which can be considered good.
...
test_result <- data.frame(
										 ...
										 res = y_true - y_pred_test,
										 res_unnorm = y_true_unnorm - y_pred_test_unnorm
										 ) %>%				
				group_by(y_true_unnorm) %>%
				mutate(
...
```{r}
print("Average Mean Percentage Absolute Error (Validation): ")
print(mean(abs(val_result$y_true_unnorm - val_result$y_pred_unnorm)))
print("Average Mean Percentage Absolute Error (Testing): ")
print(mean(abs(test_result$y_true_unnorm - test_result$y_pred_test_unnorm)))
```
...
geom_point(mapping = aes(y = y_true_unnorm - y_pred_unnorm), color = "#a8ddb5") +
geom_line(mapping = aes(y = res_unnorm_agg), color = "#fdbb84") +
labs(title="Residuals plot - validation set", x = "Groundtruth", y = "Residuals")

test_plot <- ggplot(
			 data = test_result,
			 mapping = aes(x = y_true_unnorm)) +
geom_point(mapping = aes(y = y_true_unnorm - y_pred_test_unnorm), color = "#a8ddb5") +
geom_line(mapping = aes(y = res_unnorm_agg), color = "#fdbb84") +
